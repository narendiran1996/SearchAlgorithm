...
class Nodedetail:
    ...
    def getNeighboursLexiReverse(self):
        sorted_x = sorted(self.neighbourNodeDict.items(), key=lambda kv: kv[0], reverse=True)
        return collections.OrderedDict(sorted_x)
...
def amIover(nodeName, explored):
    for nnn in list(nodeDict[nodeName].neighbourNodeDict.keys()):
        if nnn not in explored:
            return False
    return True
def DFS(fromNode, toNode):
    if os.path.exists('../DFSoutput/'):
        import shutil
        shutil.rmtree('../DFSoutput/')
    os.makedirs('../DFSoutput/')
    ...
    i=0
    cv2.imwrite('../DFSoutput/tamilDFS'+str(i)+'.jpg', cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE))
    i = i + 1
    while frontier.isEmpty() == False:
        ...
        stat = 0
        for neigh in nodeDict[currentNode].getNeighboursLexiReverse():
            if neigh not in (frontier.myStack + explored):
                stat = 1
                frontier.push(neigh)
        ...
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
print(DFS('Coimbatore', 'Pondicherry'))
print(dicDistanceFromSource)
