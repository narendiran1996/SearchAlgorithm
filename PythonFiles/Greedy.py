...
class MyPrioirtyQueue():
    ...
    def dequeue(self):
        ...
        for i in range(len(self.myQueue)):
            if dicSTLineDistanceFromALLtoGoal[self.myQueue[i]] < dicSTLineDistanceFromALLtoGoal[self.myQueue[minDistanceIndex]]:
                minDistanceIndex = i
...
def Greedy(fromNode, toNode):
    createStraightLineDistance(toNode)
    if os.path.exists('../Greedyoutput/'):
        import shutil
        shutil.rmtree('../Greedyoutput/')
    os.makedirs('../Greedyoutput/')
    ...
    i=0
    cv2.imwrite('../Greedyoutput/tamilGreedy'+str(i)+'.jpg', cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE))
    i = i + 1
    while frontier.isEmpty() == False: 
        ...
        for k in range(len(frontier.myQueue)):
            print("'"+frontier.myQueue[k]+"'"+' - '+str(int(dicSTLineDistanceFromALLtoGoal[frontier.myQueue[k]])), end=',')
        print(']')
...
print(Greedy('Coimbatore', 'Pondicherry'))

print(dicSTLineDistanceFromALLtoGoal["Coimbatore"])