...
def BFS(fromNode, toNode):
    if os.path.exists('../BFSoutput/'):
        import shutil
        shutil.rmtree('../BFSoutput/')
    os.makedirs('../BFSoutput/')
    global img
    frontier = MyQueue()
    ...
    i=0
    cv2.imwrite('../BFSoutput/tamilBFS'+str(i)+'.jpg', cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE))
    i = i + 1
...
print(BFS('Coimbatore', 'Pondicherry'))
